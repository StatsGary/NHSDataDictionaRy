...
## Using the lookup with factors
Rather than using joins, you can also rename the codes as factors using the forcats package.  

```{r factored}
act_aggregations %>%
  mutate(SpecCode = forcats::fct_recode(
    SpecCode, 
    !!!tableRtoNamedList(
      "ACTIVITY TREATMENT FUNCTION CODE", 
      table_method = "NHStableR")
    )
  )
```
...
## Use with OpenSafely Code Lists
OpenSafely is a project from TheDataLab, using a secure analytics platform for electronic health records in the NHS, created to deliver urgent results during the global COVID-19 emergency.  The project follows an open, reproducible phillosophy, and has published the codelist used for their research.  Alongside this, they have published wesbite that allows individual users to register (free of charge) and create open code lists of medicines, procedures and diagnoses.   These lists can be downloaded in CSV format and are publicly accessible.  The list is also available as an html table and can be downloaded using the NHS Data Dictionary tools.

Simply visit (https://codelists.opensafely.org)[https://codelists.opensafely.org] and find the list you wish to use.  Open the list, and on the left of the list you will see fopr example **Codelist ID:** opensafely/ace-inhibitor-medications.  OpenSafely uses clinical terminalogies that are copyright of other organisations - refer to their website for details of use.

```{r OpenSafely}
# To access the latest simply use
openSafelyCodeList("opensafely/ace-inhibitor-medications")

# To access a specific version use
openSafelyCodeList("opensafely/ace-inhibitor-medications", "2020-05-19") -> aceInhibitors

head(aceInhibitors, 6)
